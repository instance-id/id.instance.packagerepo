...
namespace instance.id.Extensions
{
    public static class InspectorStyleLine
    {
        public static VisualElement GetLine(IStyle style, PropertyInfo property, string label)
        {
            ...
            bool val;
            var editElement = GetStyleEditor(style, property, label, out val);
            toggle.RegisterValueChangedCallback((ChangeEvent<bool> evt) => { editElement.SetEnabled(evt.newValue); });
        ...
        private static VisualElement GetStyleEditor(IStyle style, PropertyInfo property, string label, out bool isActive)
        {
            ...
            var o = property.GetValue(style);
            isActive = true;
            if (t == typeof(StyleLength))
            {
                var styleLength = (StyleLength) o;
                var el = CreateField<float>(label, styleLength.value.value, (ChangeEvent<float> evt) => property.SetValue(style, new StyleLength(evt.newValue)));
            ...
            else if (t.IsGenericType && t.GetGenericTypeDefinition() == typeof(StyleEnum<>))
            {
                ...
                return CreateField<Enum>(label, val, (ChangeEvent<Enum> evt) =>
                {
            ...
            else if (t == typeof(StyleFloat))
            {
                ...
                return CreateField<float>(label, styleFloat.value, (ChangeEvent<float> evt) => property.SetValue(style, new StyleFloat(evt.newValue)));
            }
            else if (t == typeof(StyleInt))
            {
                var styleInt = (StyleInt) o;
                return CreateField<int>(label, styleInt.value, (ChangeEvent<int> evt) => property.SetValue(style, new StyleInt(evt.newValue)));
            }
            else if (t == typeof(StyleColor))
            {
                var styleColor = (StyleColor) o;
                return CreateField<Color>(label, styleColor.value, (ChangeEvent<Color> evt) => property.SetValue(style, new StyleColor(evt.newValue)));
            }
